...
namespace Long.World.Map
{
    public abstract class GameMapBase<TRole, TUser> where TRole : WorldObject
    {
        ...
        public bool IsMoveEnable(int xSour, int ySour, int x, int y, int nClimbCap)
        {
            if (!IsValidPoint(x, y))
            {
                return false;
            ...
            //if (mapData.GetRoleAmount(x, y) > 0)
            //{
            //    return false;
            //}
            if (mapData.GetFloorAlt(x, y) - mapData.GetFloorAlt(xSour, ySour) > nClimbCap)
            {
                return false;
            }
            return true;
