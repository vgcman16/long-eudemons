...
namespace Long.Shared.Mathematics
{
    public static class Calculations
    {
        ...
        {
            return Math.Sqrt(Math.Pow(posCenter.X - pos.X, 2) + Math.Pow(posCenter.Y - pos.Y, 2)) < nRange + 0.9;
        }
        ...
        public static bool IsInFan(Point pos, Point posSource, int nRange, int nWidth, Point posCenter)
        {
            ...
            {
                throw new ArgumentException("Invalid width value. Width must be greater than 0 and less than 360.");
            }
            ...
            if (GetDistance(posSource.X, posSource.Y, pos.X, pos.Y) > nRange)
            {
                return false;
            }

            const float PI = 3.1415926535f;
            double fRadianDelta = (PI * nWidth / 180) / 2;
            double fCenterLine = (double)GetRadian(posSource.X, posSource.Y, posCenter.X, posCenter.Y);
            double fTargetLine = (double)GetRadian(posSource.X, posSource.Y, pos.X, pos.Y);
        ...
        public static int POS2INDEX(int x, int y, int cx, int cy) { return (x + y * cx); }
