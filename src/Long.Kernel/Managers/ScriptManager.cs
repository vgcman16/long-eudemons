...
namespace Long.Kernel.Managers
{
    public class ScriptManager
    {
        ...
        private static readonly List<DbConfig> configs = new();
        ...
        public static async Task InitializeAsync()
        {
            ...
            configs.AddRange(await ConfigRepository.GetAsync());
        ...
        public static DbConfig GetConfig(uint idConfig, uint data = 0)
        {
            if (data == 0)
            {
                return configs.Where(x => x.Type == idConfig).FirstOrDefault();
            }

            return configs.Where(x => x.Type == idConfig && x.Data1 == data).FirstOrDefault();
        }
