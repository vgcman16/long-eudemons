...
{
    public sealed class MsgTaskDialog : MsgBase<GameClientBase>
    {
                ...
                case TaskInteraction.Answer:
                    {
                        ...
                        if (uint.TryParse(idTask, out var id))
                        {
                            ...
                            user.ClearTaskId();
                            await GameAction.ExecuteActionAsync(user.TestTask(task) ? task.IdNext : task.IdNextfail, user, targetRole, user.UserPackage.FindItemByIdentity(user.InteractingItem), Text);
                        }
                        break;
                    }
