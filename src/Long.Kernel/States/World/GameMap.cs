...
namespace Long.Kernel.States.World
{
    ...
    {
        public const uint NPC_JAIL_ID = 100;
        ...
        public async Task SendMapInfoAsync(Character user)
        {
            ...
            {
                Action = EOActionType.actionMapARGB,
                Identity = 1,
                Command = Light,
                Argument = 0
            };
        ...
        public void CollectMapThing(ref Dictionary<int, Role> setMapThing, Point pos, int nRange)
        {
            int nSize = nRange * 2 + 1;
            int nBufSize = nSize * nSize;
            setMapThing = new Dictionary<int, Role>(nBufSize);

            List<Role> targets = Query9BlocksByPos(pos.X, pos.Y);
            foreach (var role in targets)
            {
                if (role != null && Calculations.GetDistance(role.X, role.Y, pos.X, pos.Y) <= nRange)
                {
                    int idx = Calculations.POS2INDEX(role.X - (pos.X - nRange), role.Y - (pos.Y - nRange), nSize, nSize);
                    if (idx >= 0 && idx < nBufSize)
                    {
                        if (!setMapThing.ContainsKey(idx))
                            setMapThing.Add(idx, role);
                    }
                }
            }
        }
