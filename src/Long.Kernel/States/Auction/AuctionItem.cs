...
namespace Long.Kernel.States.Auction
{
    public class AuctionItem
    {
        ...
        public Character User => RoleManager.GetUser(auction.UserId);
        public ItemBase GetItem() => item;

        public uint SellerId => auction.UserId;
        public string SellerName => auction.UserName;
        ...
        public async Task<bool> InitializeAsync()
        {
            if (User == null)
            {
                DbUser user = await UserRepository.FindByIdentityAsync(auction.UserId);
                if (user == null)
                {
                ...
                item = tempItem;
                item.Position = ItemBase.ItemPosition.AuctionStorage;
        ...
        public int SocketNum
        {
            ...
            {
                int num = 0;
                num += item.SocketOne != ItemBase.SocketGem.NoSocket ? 1 : 0;
                num += item.SocketTwo != ItemBase.SocketGem.NoSocket ? 1 : 0;
                return num;
            }
        }
        ...
        public AuctionMoneyType MoneyType => AuctionMoneyType.Money;// (AuctionMoneyType)auction.MoneyType;

        public uint CurrentPrice => BiggestBid?.Price ?? auction.Price;

        public uint Buyout => 0;// auction.Buyout;

        public ushort Category => 0;// auction.Class;
        ...
        public async Task<bool> BuyoutAsync(Character buyer)
        {
            ...
            {
                if (!await buyer.SpendEudemonPointsAsync((int)Buyout, true))
                {
                    return false;
                }
        ...
        public async Task PlaceBidAsync(Character user, uint value, int amount = 1)
        {
            ...
            {
                if (!await user.SpendEudemonPointsAsync((int)payment, true))
                {
                    return;
                }
