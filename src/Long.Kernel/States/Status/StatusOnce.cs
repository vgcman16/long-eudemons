...
namespace Long.Kernel.States.Status
{
    public sealed class StatusOnce : IStatus
    {
        ...
        public async Task OnTimerAsync()
        {
            ...
            {
                logger.Error(ex, "StatusOnce.OnTimerAsync: {Message}", ex.Message);
            }
        }

        public async Task<bool> CreateAsync(Role role, int status, int power, int secs, int times, uint caster = 0, object magic = null, bool save = false)
        {
            owner = role;
            CasterId = caster;
            Identity = status;
            ...
            if (save && owner is Character)
            {
                ...
                await ServerDbContext.CreateAsync(Model);
            }
